...
int clPeak::runGlobalBandwidthTest(cl::CommandQueue &queue, cl::Program &prog, device_info_t &devInfo)
{
  ...
  try
  {
    ...
    log->print(TAB TAB "items, global_size, local_size: " NEWLINE);
    log->print(TAB TAB), log->print(numItems), log->print(TAB), log->print(numItems/FETCH_PER_WI), log->print(TAB), log->print(devInfo.maxWGSize);
    log->print(NEWLINE);
    LOGI("items: %u, global_size: %u, local_size: %u", numItems, numItems/FETCH_PER_WI, devInfo.maxWGSize);
    LOGI("input & output mem size(M): %lf", numItems * sizeof(float) * 2 / 1024 / 1024 / 1.0);
    ///////////////////////////////////////////////////////////////////////////
    // Vector width 1
    ...
    LOGI("bandwidth V1 timed local: %lf, timed global: %lf", timed_lo, timed_go);
    ...
    LOGI("bandwidth V2 timed local: %lf, timed global: %lf", timed_lo, timed_go);
    timed = (timed_lo < timed_go)? timed_lo: timed_go;
    ...
    LOGI("bandwidth V4 timed local: %lf, timed global: %lf", timed_lo, timed_go);
    timed = (timed_lo < timed_go)? timed_lo: timed_go;
    ...
    LOGI("bandwidth V8 timed local: %lf, timed global: %lf", timed_lo, timed_go);
    timed = (timed_lo < timed_go)? timed_lo: timed_go;
    ...
    LOGI("bandwidth V16 timed local: %lf, timed global: %lf", timed_lo, timed_go);
    timed = (timed_lo < timed_go)? timed_lo: timed_go;
